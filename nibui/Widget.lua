...
function Widget:new(config, document, parent)
    ...
    for k, _ in lang.zip(config, defaults) do
        if config[k] then
...
function Widget:clip_box(b)
    ...
    if parent ~= self and parent ~= nil and parent.clip_box then
        ...
        parent_box[1] = parent_box[1] + b
        parent_box[2] = parent_box[2] + b
        parent_box[3] = parent_box[3] + 2*b
        parent_box[4] = parent_box[4] + 2*b
...
function Widget:draw_3slice(x, y, w, h, sx, sy, sw, sh, sl1, sl2)
    ...
    love.graphics.setScissor(x+sl1, y, w-(sw-step), h)
...
function Widget:draw()
    -- FIXME: use canvases instead of always drawing
    self.dirty = true
    ...
    if self.dirty then
        ...
        --love.graphics.setScissor(unpack(self:clip_box()))

        if z ~= 0 then
            -- FIXME: use 24bit color
            love.graphics.setColor(shadow_color, shadow_color, shadow_color)

            love.graphics.rectangle("fill", x+r, y+z, w-r*2, h)
            love.graphics.rectangle("fill", x, y+r+z, w, h-r*2)

            -- FIXME: fill circle
            -- if r ~= 0 then
            --     love.graphics.rectangle("fill", x+r, y+r+z, r)
            --     love.graphics.rectangle("fill", x+w-r-1, y+r+z, r)
            --     love.graphics.rectangle("fill", x+r, y+h-r+z-1, r)
            --     love.graphics.rectangle("fill", x+w-r-1, y+h-r+z-1, r)
            -- end
        end

        love.graphics.setColor(border_color, border_color, border_color)
        ...
        -- FIXME: fill circle
        -- if r ~= 0 then
        --     love.graphics.rectangle("fill", x+r, y+r, r)
        --     love.graphics.rectangle("fill", x+w-r-1, y+r, r)
        --     love.graphics.rectangle("fill", x+r, y+h-r-1, r)
        --     love.graphics.rectangle("fill", x+w-r-1, y+h-r-1, r)
        -- end

        if type(self.background) ~= 'table' then
            local background = self.background

            love.graphics.setColor(background, background, background)
            ...
            -- FIXME: port sprite drawing routines
            ...
            --     local sprite_x, sprite_y, sprite_w, sprite_h = unpack(self.background)

            --     custom_sprite(x, y, sprite_x, sprite_y, sprite_w, sprite_h, self.palette)
            -- elseif #self.background == 6 then
            --     self:draw_3slice(x, y, w, h, unpack(self.background))
            -- elseif #self.background == 8 then
            --     self:draw_9slice(x, y, w, h, unpack(self.background))
            -- end
        end
        ...
        -- FIXME: port text coloring and drawing to love2d
...
function Widget:in_point(ex, ey)
    local x, y, w, h = unpack(self:clip_box())
