...
-- FIXME: dynamic screen size! Or get from Love2d
local DEFAULT_W = 800
local DEFAULT_H = 600

local NOM = {}

local shift = function(tbl) return table.remove(tbl, 1) end
...
function NOM:draw()
    self.root:draw()
    love.graphics.setScissor(0, 0, DEFAULT_W, DEFAULT_H)
...
function NOM:do_fire()
    table.sort(self.event_queue, function (a, b)
    ...
    repeat
        event = table.remove(self.event_queue)
